...
在这个脚本文件中，我们可以看到它定义了两个符号集合，分别为"SUNW_1.1"和"SUNWprivate"（在Solaris系统中，符号的集合名通常由"SUNW"开头）。第一个包含了两个全局符号pop和push；在第二个集合中，包含了两个全局符号"\_\_pop"和"\_\_push"。第二个集合中最后的"local:
